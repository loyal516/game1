...
| **시간 종료 (생존)** | 생존자 수 우세. 동률이면 점령 수 우세, 다시 동률이면 무승부 | 생존/점령 우세 팀 |
...
| **협동 조건** | 포획자는 해당 대상을 붙들고 있는 플레이어와 달라야 함 |
| **조작** | F키 (길게 누르기 1.5초) |
| **캔슬** | 피격, 키 해제, 대상 탈출 |
| **성공 시** | 대상 게임에서 제외 |
...
|-----------|----------------|-----------------|
| [Dead by Daylight](https://forums.bhvr.com/dead-by-daylight/kb/articles/369-developer-update-january-2023) | Survivor Activity HUD처럼 팀원의 현재 행동과 chase 여부를 portrait 주변에 붙이면, 보이스 없는 솔로큐도 다음 행동을 판단할 수 있다. | 좌측 팀 상태 레일에 `중립/공격자/왕/붙들림/포획됨`과 `구출 중/점령 중/추격 중` 행동 아이콘을 붙이고, 붙들림/구출/포획 진행률은 아이콘 링으로 표시한다. |
| [Among Us](https://www.innersloth.com/games/among-us/) | task bar, emergency meeting, sabotage처럼 팀 목표와 긴급 이벤트를 아주 단순한 문장/버튼/맵 정보로 반복 노출한다. | 포획/구출/점령/왕 박탈은 중앙 배너와 우측 objective panel에 같은 용어로 표시한다. `포기`, `시간 종료`, `전원 포획` 결과도 결과 배너 문구를 짧게 유지한다. |
| [Splatoon 3](https://splatoon.nintendo.com/en/news/beginner-basics-for-splatoon-3-the-ins-and-outs-of-playing-online/) | Turf War/Splat Zones처럼 색상으로 전장 점유 상태를 즉시 읽게 하고, 모드별 목표가 UI 언어와 직접 연결된다. | 스태미나와 슬라임 지속시간을 캐릭터 주변 또는 하단 HUD에 붙여 추격 중 시선 이동을 줄인다. 점령 포인트는 팀 색상+형태를 같이 써 색각 의존을 낮춘다. |
...
| [Apex Legends / EA Accessibility Patent Pledge](https://www.ea.com/commitments/positive-play/accessibility-patent-pledge) | 컨트롤러에 매핑 가능한 입력으로 상황 인식형 오디오/비주얼 커뮤니케이션을 만든다. 보이스/텍스트가 어려운 플레이어도 같은 전술 정보를 보낼 수 있어야 한다. | 핑은 월드 마커+미니맵 마커+짧은 팀 로그를 동시에 생성한다. 솔로큐를 위해 자주 쓰는 핑은 음성 없이도 의미가 명확해야 하며, 핑 음량/표시 끄기 옵션을 접근성 backlog에 둔다. |
| [The Finals](https://www.reachthefinals.com/patchnotes/3140) / [Spectator HUD](https://www.reachthefinals.com/patchnotes/890) | Cashout/Vault 시각 지표, team HUD 정보, spectator HUD처럼 목표물과 팀 상태를 같은 화면 언어로 묶는다. | 점령 포인트 핑은 포인트 ID, 점령 상태, 예상 충돌 위험을 함께 보여준다. 관전 HUD는 생존 아군, 포획 상태, 점령 상태를 한 화면에서 읽게 한다. |

#### 9.6.1 벤치마크에서 확정한 UI 규칙

| 규칙 | 근거 | Overthrone 요구사항 |
|------|------|---------------------|
| 솔로큐 정보 격차를 줄이되 위치를 과하게 주지 않는다 | Dead by Daylight의 Survivor Activity HUD는 행동/추격 상태를 portrait 주변에 붙이지만 정확한 위치 노출은 피한다. | 좌측 팀 상태 레일은 행동 상태와 위기 상태를 보여주되, 일반 아군의 정확한 좌표는 미니맵에 항상 노출하지 않는다. 적 왕/공격자/핑처럼 게임 규칙상 노출되는 정보만 위치화한다. |
| 탭은 컨텍스트, 홀드는 선택형 휠로 나눈다 | Overwatch 2는 조준 대상 기반 컨텍스트 핑과 홀드 휠, 응답 인터랙션을 같이 둔다. | `G` 탭은 적/점령/전방 주목 자동 핑, `G` 홀드는 Going/Defend/Capture/Help 휠, 팀원 응답은 같은 마커에 누적한다. |
| 점령 정보는 색만 쓰지 않는다 | Splatoon 3는 영역/중앙 점령/카운트다운을 맵과 전장 색으로 반복 노출한다. | CapturePoint UI는 팀 색상, 포인트 ID, progress fill, state text, occupant count를 같이 표시한다. 색각 접근성을 위해 아이콘 형태와 텍스트를 병행한다. |
| 관전 HUD는 경기 읽기를 도와야 한다 | THE FINALS는 관전 전용 team HUD, health bar, 거리/월드 아이콘 표시 옵션을 별도 관리한다. | Captured 관전 HUD는 현재 관전 대상, 생존 아군, 포획/붙들림 상태, 점령 현황, 핑 로그를 한 화면에서 읽게 하되 전투 화면을 가리지 않게 compact 모드로 둔다. |
| 핑은 접근성 기능으로 취급한다 | EA의 접근성 특허 pledge는 contextual audio/visual communication을 mappable input 기반 접근성 기술로 설명한다. | 핑/팀 로그/상태 아이콘은 키 리바인딩, 음량, 시각 표시 강도, 스팸 보호를 설정 backlog에 포함한다. |
...
#### 9.6.4 UI 벤치마크 적용 수용 기준

**목표:** 레퍼런스 게임을 단순히 "비슷하게 보이게" 참고하지 않고, Overthrone의 HUD/핑/관전 UI가 실제 플레이 판단을 빠르게 만드는지 검증한다.

| UI surface | 벤치마크 근거 | Overthrone 수용 기준 | 로컬 검증 방식 |
|------------|---------------|----------------------|----------------|
| 팀 상태 레일 | [Dead by Daylight Survivor Activity HUD](https://forums.bhvr.com/dead-by-daylight/kb/articles/369-developer-update-january-2023)는 팀원의 행동과 chase 여부를 portrait 주변에 붙여 솔로큐 정보 격차를 줄인다. | 좌측 팀 상태 레일은 각 아군의 `상태`, `붙들림/포획 위험`, `구출/점령/추격 행동`을 한눈에 구분할 수 있게 계층화한다. 정확한 월드 좌표는 항상 공개하지 않고, 규칙상 공개되는 적 왕/공격자/핑만 위치화한다. 0.5초 판독 목표는 PlayMode 관찰/플레이테스트 체크 항목으로 분리한다. | 1280x720, 1280x576, 1920x1080에서 팀 레일 텍스트/아이콘 겹침 없이 표시. Held/Captured/Slime/King/Attacker 상태 전환 EditMode 테스트 + PlayMode 스크린샷 체크. |
| Objective panel | [Splatoon 3](https://splatoon.nintendo.com/en/gameplay/)는 팀 색상과 영역 점유를 반복 노출하고, [Beginner Basics](https://splatoon.nintendo.com/en/news/beginner-basics-for-splatoon-3-the-ins-and-outs-of-playing-online/)는 맵으로 팀별 영역을 확인하게 한다. | 점령 포인트 A/B/C는 색상만으로 구분하지 않고 `ID`, `owner`, `progress fill`, `경합/감소/완료 상태`, `Blue/Red count`를 함께 보여준다. 3점 점령 30초 카운트다운은 중앙 배너와 우측 panel에 같은 용어로 반복한다. | CapturePoint owner/progress/count 변화를 `PlayerHud`에 주입하는 EditMode 테스트. 색각 접근성을 위해 색상 없이 텍스트/형태만으로도 owner/state를 구분하는 스크린샷 리뷰. |
| 핑/커뮤니케이션 | [Overwatch 2 ping system](https://overwatch.blizzard.com/en-us/news/23785337/an-inside-look-at-the-ping-system-in-overwatch-2/)은 조준 대상 기반 contextual ping과 hold wheel을 결합하고, [EA accessibility patent pledge](https://www.ea.com/en-au/commitments/positive-play/accessibility-patent-pledge)는 mappable input 기반 contextual audio/visual communication을 접근성 기술로 분류한다. | `G` 탭은 조준/컨텍스트 기반 자동 핑, `G` 홀드는 Going/Defend/Capture/Help 휠이다. 핑 데이터는 월드 마커, 미니맵 마커, 팀 로그를 동시에 만들고, 같은 마커에 팀 응답을 누적할 수 있어야 한다. 스팸 방지 cooldown과 표시/음량 옵션은 접근성 backlog에 둔다. | `LocalPingSystem` EditMode 테스트로 컨텍스트 우선순위, 응답 데이터 누적, TTL 만료 검증. 로컬-first에서는 RPC 없이 같은 scene 참가자 배열에만 브로드캐스트한다. |
| 관전 HUD | [THE FINALS spectator HUD update](https://www.reachthefinals.com/patchnotes/890)는 spectator용 team HUD와 steal/cashout 정보를 더 명확히 보여준다. | Captured 관전자는 현재 관전 대상, 생존 아군, Held/Captured 상태, 점령 현황, 핑 로그를 compact mode로 본다. 관전 HUD는 전투 화면 중앙을 가리지 않고, `Q`/`Tab` 관전 대상 순환 입력과 항상 일치해야 한다. | `LocalSpectatorCamera` 대상 순환 테스트 + HUD overlay 표시 테스트. 1280x576 낮은 화면에서 objective panel/관전 overlay가 겹치지 않는 스크린샷 체크. |
| 긴급 이벤트/결과 배너 | [Among Us](https://www.innersloth.com/games/among-us/)는 task bar, emergency meeting, sabotage처럼 팀 목표와 긴급 이벤트를 짧은 문장/버튼/맵 정보로 반복 노출한다. | 각 이벤트명(`전원 포획`, `3점 점령`, `시간 종료`, `포기`, `왕 박탈`, `공수전환`)은 중앙 배너, objective panel, 로그에서 동일한 용어로 반복한다. 이벤트끼리는 서로 구분되는 짧은 문구를 유지한다. | `LocalMatchFlowPresenter` EditMode 테스트에서 event type별 banner text와 HUD phase text 일치 검증. |

#### 9.6.5 UI 2차 구현 체크리스트

| 우선순위 | 항목 | 완료 조건 | 남은 리스크 |
|----------|------|-----------|-------------|
| P0 | 팀 상태 아이콘 아트 | 로컬 MVP 완료: 팀 상태 레일에 Captured/Held/Holding/King/Attacker/Slime/Neutral별 절차적 shape sprite와 색상을 표시하고 stale slot을 숨김 | 정식 아이콘 에셋, 색각 모드 스크린샷 QA, compact layout 실측 필요 |
| P0 | 핑 응답 상호작용 polish | 로컬 MVP 완료: 같은 핑에 누적된 `간다/방어/도움/점령` 응답이 팀 로그에 묶여 표시되고, 최신 응답자가 `LATEST`로 표시됨 | 온라인 전환 시 RPC/권위 경계 재설계 필요 |
| P1 | Objective panel compact mode | 1280x576에서도 A/B/C, countdown, phase, defender text가 겹치지 않음 | 좁은 화면용 레이아웃 분기 필요 |
| P1 | 관전 HUD compact mode | Captured 상태에서 조작 안내, 대상, 팀 상태, objective가 동시에 읽힘 | 정식 카메라 컷인/사운드 추가 시 재배치 필요 |
| P2 | 접근성/스팸 보호 옵션 | 핑 표시 강도, 핑 음량, UI scale, colorblind-safe palette, ping cooldown/spam protection을 설정 backlog에 연결 | 실제 옵션 UI는 온라인/설정 화면 단계에서 구현 |
...
### 19.4 현재 구현 상태 (2026-07-04)

**현재 기준:** `unity/OverthroneUnity` 로컬 Unity prototype. 외부 Photon/Supabase/Steam 의존 없이 Unity Editor와 EditMode/PlayMode smoke 테스트로 검증한다.
...
| 상태별 이동 | 완료 | Neutral/Attacker/King/Held/Captured/Slime/Holding MovementProfile, Held/Captured/Holding 수평 속도 즉시 0, 로컬 Blue 3/Red 3 roster, 점령 수 기반 팀당 King 1명, 최종 포획 수→점령 기여도→tie-breaker 왕 승계, 3명 집결 기반 Attacker, 5초 Attacker 유예, 시작 스폰이 점령 반경과 겹치지 않도록 배치 | Photon roster 동기화 |
...
| HUD/UI | 부분 완료 | 크로스헤어, 스태미나 바, 상태 텍스트, Dash/Slime 쿨다운/스태미나 부족 표시, 우측 Objective 점령/소유/승리 카운트다운/phase/defender 텍스트, 우측 점령 패널 MVP(A/B/C row, owner 색상, progress fill, state, Blue/Red count), 좌측 팀 상태 레일, 상태별 절차적 팀 상태 아이콘, 포획 진행 링, 붙들림/구출 전용 링, 관전 overlay, 로컬 미니맵 panel/자신/아군/적 왕/적 공격자/점령 포인트/핑 마커, `G` 탭 컨텍스트 핑, `G` 홀드 방사형 핑 휠, 로컬 핑 응답 누적 로그/최신 응답자 표시, EditMode 테스트 | 정식 아이콘 아트/compact QA, 온라인 RPC 동기화 |
| 로컬 봇/AI hearing | 부분 완료 | `LocalBotController` 5명 NPC NavMesh path corner 추종 + direct fallback, 점령 포인트 선택, Attacker/King 추격·덮치기, Held 아군 구출 이동, King이 아군이 붙든 적만 최종 포획 보조, NavMesh/direct fallback 공통 dynamic obstacle avoidance MVP, FOV/range/LOS 기반 sight suspicion memory MVP, 달리기 발소리 NoiseEvent, 바닥 `FootstepSurface`별 volume/pitch/noiseRadius multiplier, AIHearingSensor 소음 기억, 적 팀 소음 위치 조사, 도착 후 조용한 수색 waypoint 순환, 짧은 경계, 같은 소리 소비 처리 MVP, EditMode 테스트, PlayMode smoke 테스트, 6봇 choke/direct fallback/dynamic avoidance 240프레임 PlayMode soak fixture 테스트 | 실제 씬 기반 장시간 soak/occluder 반복 검증, 정식 표면별 오디오 클립/믹싱 |
| 애니메이션 | 부분 완료 | Placeholder Idle/Walk/Run Blend Tree | 실제 모델/모션 클립, 점프/앉기/덮치기/슬라임 애니메이션 |
| 점령/승리 시스템 | 부분 완료 | 로컬 CapturePoint 3개, 반경 5m, 팀별 인원 집계, 1명 5%/초·2명 8%/초·3명+ 12%/초, 경합 정지, 빈 미완료 포인트 3%/초 decay, Attacker/King 자동 부여, 3점 점령 30초 승리 카운트다운, 상대 팀 전원 Captured 즉시 승리, 양 팀 roster 존재 시 active/enabled 0명 팀의 포기 패배, 시간 종료 생존자 수→owned point 수→draw 판정, `LocalMatchPhase` Playing/VictoryCountdown/Result, defender re-entry window, 카운트다운 시작/중단/종료 `LocalMatchFlowEvent`, HUD phase/defender 표시, `LocalMatchFlowPresenter` 배너/overlay/defender re-entry spawn/임시 Attacker 버프, Winner `None` draw 배너, EditMode 테스트 | 정식 카메라 컷인, 공수전환 사운드/아트 polish, 네트워크 권위화 |
| 포획/구출 루프 | 부분 완료 | 로컬 `PlayerCaptureAgent`/`LocalCaptureSystem`, Attacker/King 덮치기 조건, `TackleHitbox` 전방 물리 히트박스, Holding/Held 상태, 덮치기 실패 시 0.5초 경직, 왕 1.5초 최종 포획, 붙든 자와 포획자 분리 규칙, 같은 팀 접촉 구출, 구출/슬라임 탈출/붙드는 자 피격 입력 시 붙들림 해제 + 붙든 자 1초 경직, Captured 시 `LocalSpectatorCamera` 자동 관전 전환, `Q`/`Tab` 관전 대상 순환 입력, 관전 HUD overlay, `LocalDeadChannel` Captured 전용 같은 팀 로그/최종 포획 system join 메시지, Captured 전용 dead channel 입력 UI, `CaptureFeedbackController` 절차적 VFX/SFX, EditMode 테스트 | 정식 포획 VFX/SFX 에셋/믹싱, 온라인 dead channel 채팅/moderation |
| 슬라임 스킬 | 부분 완료 | R 입력 시 스태미나 50을 소모하고 15초 쿨다운이 시작되는 임시 Slime movement state, GDD 기준 9.8m/s 고기동, Held 상태에서 매치당 1회 자가 탈출, `CharacterController` 히트박스 축소, no-input 감속 완화로 미끄러짐 처리, placeholder squash/widen 변형 비주얼, HUD 쿨다운/스태미나 gate 표시, EditMode 테스트 | 정식 shader/softbody VFX, 좁은 통로 레벨 검증, 라운드 재시작 시 탈출권 reset hook |
...
1. **포획 루프 보강:** 정식 포획 VFX/SFX 에셋/믹싱, 온라인 dead channel 채팅/moderation.
2. **UI 2차:** 정식 아이콘 아트/compact QA, 온라인 RPC 동기화.
