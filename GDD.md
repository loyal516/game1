...
| 로컬 봇/AI hearing | 부분 완료 | `LocalBotController` 5명 NPC NavMesh path corner 추종 + direct fallback, 점령 포인트 선택, Attacker/King 추격·덮치기, Held 아군 구출 이동, King이 아군이 붙든 적만 최종 포획 보조, 달리기 발소리 NoiseEvent, 바닥 `FootstepSurface`별 volume/pitch/noiseRadius multiplier, AIHearingSensor 소음 기억, 적 팀 소음 위치 조사, 도착 후 조용한 수색 waypoint 순환, 짧은 경계, 같은 소리 소비 처리 MVP, EditMode 테스트 | dynamic obstacle avoidance, 시야/의심도 결합, PlayMode 장시간 검증, 정식 표면별 오디오 클립/믹싱 |
