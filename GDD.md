...
| 로컬 봇/AI hearing | 부분 완료 | `LocalBotController` 5명 NPC NavMesh path corner 추종 + direct fallback, 점령 포인트 선택, Attacker/King 추격·덮치기, Held 아군 구출 이동, King 최종 포획 보조, 달리기 발소리 NoiseEvent, AIHearingSensor 소음 기억, EditMode 테스트 | dynamic obstacle avoidance, 경계/수색 상태, PlayMode 장시간 검증, 바닥 재질별 소리 |
