...
`LocalBotController`는 로컬 3v3의 NPC 5명을 실제로 움직입니다. 봇은 `PlayerInputReader.SetManualInput(...)`으로 `PlayerMotor`에 이동 입력을 주입하므로 플레이어와 같은 이동/상태 프로필을 통과합니다. 기본 행동은 점령 포인트 이동, Attacker/King 상태에서 적 추격 및 덮치기, 붙들린 아군 구출 이동, King 상태에서 Held 적 최종 포획 보조입니다. 아직 NavMesh 경로 탐색이나 행동 트리는 아니며, 로컬 플레이 가능성 검증용 직선 이동 MVP입니다.
...
걷기는 AI에게 들리지 않고, 달리기 상태에서만 `PlayerNoiseEmitter`가 발소리를 재생하고 `NoiseSystem.Emit(...)`을 호출합니다. 상태별 `noiseRadius` 값으로 AI 감지 반경을 조절합니다.
...
- 로컬 봇은 직선 이동/점령/추격/구출/포획 MVP까지이며, NavMesh 경로 탐색, 장애물 회피, 경계/수색 행동 트리는 다음 단계입니다.
