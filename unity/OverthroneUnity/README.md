...
`LocalBotController`는 로컬 3v3의 NPC 5명을 실제로 움직입니다. 봇은 `PlayerInputReader.SetManualInput(...)`으로 `PlayerMotor`에 이동 입력을 주입하므로 플레이어와 같은 이동/상태 프로필을 통과합니다. 기본 행동은 점령 포인트 이동, Attacker/King 상태에서 적 추격 및 덮치기, 붙들린 아군 구출 이동, King 상태에서 Held 적 최종 포획 보조입니다. NavMesh가 있으면 목표까지 complete path를 계산해 다음 path corner를 향해 이동하고, NavMesh sample/path 계산이 실패하면 기존 direct target 이동으로 fallback합니다.
...
- 로컬 봇은 NavMesh path corner 추종 + direct fallback, 점령/추격/구출/포획 MVP까지이며, dynamic obstacle avoidance, 경계/수색 상태, PlayMode 장시간 검증은 다음 단계입니다.
